...
for ($i=101; $i -le 112; $i++ ){
        ...
        New-AzVirtualNetworkGatewayConnection -Name "Conn-$i" -ResourceGroupName "emulated-on-premises-rg" -Location "West US 2" -VirtualNetworkGateway1 $vngop -LocalNetworkGateway2 $lng -ConnectionType IPsec -SharedKey "Enter-the-shared-key"
}